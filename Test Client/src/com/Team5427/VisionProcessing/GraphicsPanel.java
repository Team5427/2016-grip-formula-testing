...
//			gameTimerEnd = System.currentTimeMillis() + Config.AUTO_TIME * 1000;
	...
	public void initializeCalibration() {
		...
		if (Config.ENABLE_FOV_CALIBRATION && Main2.goals != null && Main2.goals.size() == 1) {
			Goal g = Main2.goals.get(0);
			...
			else if (Main2.goals.size() > 1)
				System.out.println("There are " + Main2.goals.size()
						+ " goals. Only 1 must visible in the camera for calibration.");
			else if (Main2.goals.size() == 0) {
	...
	public synchronized void paint(Graphics g) {
		...
		int x1, y1, x2, y2;
		for (int i = 0; i < Main2.goals.size(); i++) {
			...
			bg.drawLine((int) Main2.goals.get(i).getCenterLine().getX1(),
					(int) Main2.goals.get(i).getCenterLine().getY1(), (int) Main2.goals.get(i).getCenterLine().getX2(),
					(int) Main2.goals.get(i).getCenterLine().getY2());

			// bg.setColor(Color.BLUE);
			bg.drawLine((int) Main2.goals.get(i).getLeftLine().getX1(), (int) Main2.goals.get(i).getLeftLine().getY1(),
					(int) Main2.goals.get(i).getLeftLine().getX2(), (int) Main2.goals.get(i).getLeftLine().getY2());

			// bg.setColor(Color.RED);
			bg.drawLine((int) Main2.goals.get(i).getRightLine().getX1(), (int) Main2.goals.get(i).getRightLine().getY1(),
					(int) Main2.goals.get(i).getRightLine().getX2(), (int) Main2.goals.get(i).getRightLine().getY2());
		...
		for (int i = 0; i < Main2.goals.size(); i++) {
			...
			/*
			 * int x = (int) Main2.goals.get(i).getCenterLine().getX1() - 8; int
			 * y = (int) Main2.goals.get(i).getCenterLine().getY1() + 15;
			 */
			int x = xStart * i + 10;
			int y = yStart + 50;
			...
			String distance = String.format("%.2f", Main2.goals.get(i).getGoalDistanceCamera());
			String distnaceToBase = String.format("%.2f", Main2.goals.get(i).getGoalDistanceCamera());
			String angleDegrees = String.format("%.2f", Math.toDegrees(Main2.goals.get(i).getAngleOfElevation()));
			String horizontalAngle = String.format("%.2f", Math.toDegrees(Main2.goals.get(i).getTurretXAngle())); //used to be getCameraAngle, damn it Charlie
			...
			bg.drawString("Horizontal Angle: " + horizontalAngle + "°", x, y += interval);
			bg.drawString("Angle Check: " + Main2.goals.get(i).getAngleStatus() , x, y+=interval);
...
//		bg.fillRect(getWidth()/2-5, 0, 10,(int) RESOLUTION.getHeight());
		...
		if (gameTimerEnd > System.currentTimeMillis()) {
			...
			// bg.drawString("Time ReMain2ing: " + timeMinutes + ":" +
			// timeSecondTens + (timeSecondsOnes % 10), 509, 600);

		} else {
			// bg.drawString("Time ReMain2ing: 0:00" , 509, 600);
		...
		bg.drawString("FPS: " + Main2.FPS, 2, 14);
